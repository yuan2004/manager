...
jQuery.fn.colorPicker = function(options){
	...
	var OFFSET        = 3;
	var BORDER_WIDTH  = 1;
    var COLORS        = ['ff8080','ffff80','80ff80','00ff80','80ffff','0080ff','ff80c0','ff80ff',
	...
	for(var c in COLORS){
		html += '<li><a href="javascript:void(\'#'+COLORS[c]+'\');" ref="#'+COLORS[c]+'" style="background-color:#'+COLORS[c]+';"></a></li>';
	}
	html += '</ul><form style="margin:0;padding:3px;clear:both;text-align:center;">'+
			'RBG: <input id="_seven_color_code" maxlength="7" size="10" style="font:10px verdana" /> '+
	...
	.find('div').css({'background-color':'#222222','border':BORDER_WIDTH+'px solid #FFF','margin':0,'padding':OFFSET})
	.width(NUMS_PRE_LINE*(ITEM_SIZE+ITEM_OFFSET*2+BORDER_WIDTH*2))
	...
	.mousedown(function(e){
		...
		setColor(color);options.callback.call(this, e, color);
	})
	.mouseover(function(){$(this).css({'border':BORDER_WIDTH+'px solid #333'});})
	.mouseout(function(){$(this).css({'border':BORDER_WIDTH+'px solid #ccc'});});
	PICKER = result;
		...
		if(myPicker.attr('ref') != FLAG){
			$(this).hide().after(
				...
				.mouseover(function(){
					...
					var top  = offset.top+$(this).height()+BORDER_WIDTH;
					jQuery.SCP_Target = $(this).prev();
					jQuery.SCP_Active = $(this);
					PICKER.show().css({'left':left,'top':top}).find('#_seven_color_code').val(jQuery.SCP_Target.val()).focus().select();
				}).attr('readonly','true')
				.css({'border':BORDER_WIDTH+'px solid #999','cursor':'pointer','padding':0,'background-color':$(this).val()})
			);
		}
	});
}